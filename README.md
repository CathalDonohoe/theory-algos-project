...
They calculate hash values(digests) from the data (messages).
...
•	Bit-dependency: A hash algorithm is designed to ensure that each bit of the output is dependent upon every bit in the input. This prevents anyone from splitting the algorithm up and trying to reverse calculate an input from each bit of the output has separately. To solve an output bit, you must know the full input, requiring the attempt to know all or nothing. <br>
...
With SHA-512, a message digest algorithm or a hash function, is a procedure that maps input data of an arbitrary length to an output of fixed length. Output is often known as hash values or hash digests [8]. These digests are a series of numbers and characters. The length of these digests, in the case of SHA-512, are 128 characters. This means that there is only a certain amount of possible digest outputs, more than the variable of inputs. Therefore, we may result with the same output for 2 different inputs. <br>
With this question, I am going to refer to what Bitcoin miners are currently doing to mine bitcoin. In cryptocurrency  mining, a target hash is a numeric value that a hashed block header must be less than or equal to in order for a new block to be awarded to a miner [9]. Miners make these guesses by randomly generating as many "nonces" as possible, as fast as possible [10]. 
A nonce is short for "number only used once". Nonces are often used on authentication protocols and cryptographic hash functions [11]. The first miner who's nonce generates a hash that is less than or equal to the target hash is awarded credit for completing that block and is awarded the spoils of 6.25 BTC [10]. 
